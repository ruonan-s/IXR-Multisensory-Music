...
public class ElementActivator : MonoBehaviour
{
    ...
    private void SpawnElements()
    {
        ...
        foreach (var element in activeParagraph.elements)
        {
            ...
            if (prefab != null)
            {
                ...
                float minY, maxY;
                GetSectionBounds(element.section, out minY, out maxY);
                
                // 3. Calculate initial Y position (center of section)
                float initialY = (minY + maxY) / 2f;
                Vector3 initialPosition = new Vector3(0f, initialY, zPosition);
                
                float finalScaleMultiplier = element.scale.scale_multiplier * layerScaler;
                float checkRadius = SpiralIncrement * finalScaleMultiplier * CheckRadiusMultiplier;
                
                // 4. Find a safe position within section bounds
                Vector3 finalSpawnPosition = FindSafePosition(initialPosition, checkRadius, minY, maxY);
                
                // 5. Instantiate at the safe position
                ...
                Debug.Log($"[SPAWN SUCCESS] {element.image_name} (Layer: {element.layer}, Section: {element.section}, Z: {zPosition:F0}) @ {finalSpawnPosition}");
            }
            else
            {
    ...
    private void GetSectionBounds(int section, out float minY, out float maxY)
    {
        if (section == 0)
        {
            minY = 0f;
    ...
    private Vector3 FindSafePosition(Vector3 initialPos, float checkRadius, float minY, float maxY)
    {
        ...
        for (int i = 0; i < maxAttempts; i++)
        {
            ...
            // Clamp Y position to section boundaries
            currentPos.y = Mathf.Clamp(currentPos.y, minY, maxY);
            
            if (!Physics.CheckSphere(currentPos, checkRadius))
            {
                return currentPos; // Found a safe position within bounds
            }
        ...
        // Fallback: return initial position clamped to bounds
