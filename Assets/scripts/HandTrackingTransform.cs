...
public class HandTrackingTransform : MonoBehaviour
{
    [Header("References")]
    ...
    // Transform modes (toggle between these)
    private enum TransformMode { Position, Rotation }
    private TransformMode currentTransformMode = TransformMode.Position;
    
    // Interaction modes
    private enum InteractionMode { None, SingleHand, BothHands }
    ...
    private float movementMultiplier = 6.0f;  // Amplify hand movement for larger object movement
    
    // Rotation state
    private Quaternion initialHandRotation;
    private Quaternion initialCubeRotation;
    private float rotationMultiplier = 1.3f;  // Amplify rotation for more responsive control
    ...
    // Mode toggle (double pinch detection)
    private int leftPinchTapCount = 0;
    private int rightPinchTapCount = 0;
    private float lastLeftPinchTime = 0f;
    private float lastRightPinchTime = 0f;
    private float doublePinchWindow = 0.5f;  // Time window for double tap
    private bool wasIndexPinchingLeft = false;
    private bool wasIndexPinchingRight = false;
    ...
    void DetectDoublePinch()
    {
        // Left hand double pinch detection
        if (leftHand.IsTracked)
        {
            float leftPinchStrength = leftHand.GetFingerPinchStrength(OVRHand.HandFinger.Index);
            bool isCurrentlyPinching = leftPinchStrength > pinchThreshold;
            
            // Detect pinch tap (quick pinch and release)
            if (!wasIndexPinchingLeft && isCurrentlyPinching)
            {
                // Pinch started
                float timeSinceLastTap = Time.time - lastLeftPinchTime;
                
                if (timeSinceLastTap < doublePinchWindow)
                {
                    // Second tap within window - double pinch detected!
                    ToggleTransformMode();
                    leftPinchTapCount = 0;  // Reset
                    lastLeftPinchTime = 0f;
                }
                else
                {
                    // First tap
                    leftPinchTapCount = 1;
                    lastLeftPinchTime = Time.time;
                }
            }
            else if (wasIndexPinchingLeft && !isCurrentlyPinching)
            {
                // Pinch released - reset if window expired
                if (Time.time - lastLeftPinchTime > doublePinchWindow)
                {
                    leftPinchTapCount = 0;
                }
            }
            
            wasIndexPinchingLeft = isCurrentlyPinching;
        }
        
        // Right hand double pinch detection
        if (rightHand.IsTracked)
        {
            float rightPinchStrength = rightHand.GetFingerPinchStrength(OVRHand.HandFinger.Index);
            bool isCurrentlyPinching = rightPinchStrength > pinchThreshold;
            
            // Detect pinch tap (quick pinch and release)
            if (!wasIndexPinchingRight && isCurrentlyPinching)
            {
                // Pinch started
                float timeSinceLastTap = Time.time - lastRightPinchTime;
                
                if (timeSinceLastTap < doublePinchWindow)
                {
                    // Second tap within window - double pinch detected!
                    ToggleTransformMode();
                    rightPinchTapCount = 0;  // Reset
                    lastRightPinchTime = 0f;
                }
                else
                {
                    // First tap
                    rightPinchTapCount = 1;
                    lastRightPinchTime = Time.time;
                }
            }
            else if (wasIndexPinchingRight && !isCurrentlyPinching)
            {
                // Pinch released - reset if window expired
                if (Time.time - lastRightPinchTime > doublePinchWindow)
                {
                    rightPinchTapCount = 0;
                }
            }
            
            wasIndexPinchingRight = isCurrentlyPinching;
        }
    }
    ...
    {
        currentTransformMode = currentTransformMode == TransformMode.Position 
            ? TransformMode.Rotation 
            : TransformMode.Position;
        
        // Reset auto-confirm when mode changes
        timeAtThreshold = 0f;
        wasWithinThreshold = false;
        
        string modeName = currentTransformMode == TransformMode.Position ? "POSITION" : "ROTATION";
        Debug.Log($"Switched to {modeName} mode");
        UpdateGuideText();
    }
    ...
    void DetermineInteractionMode()
    {
        ...
        // Priority 2: One pinching = Position or Rotation (depends on current transform mode)
        else if (isLeftPinching || isRightPinching)
        {
            if (previousInteractionMode != InteractionMode.SingleHand)
            {
            ...
            currentInteractionMode = InteractionMode.SingleHand;
        }
        else
        {
            currentInteractionMode = InteractionMode.None;
    ...
    {
        usingLeftHand = isLeftPinching;
        initialGrabHandPos = usingLeftHand ? leftPinchPos : rightPinchPos;
    ...
    {
        usingLeftHand = isLeftPinching;
        initialHandRotation = usingLeftHand ? leftHandRotation : rightHandRotation;
    ...
    void ExecuteInteraction()
    {
        ...
        else if (currentInteractionMode == InteractionMode.SingleHand)
        {
            if (currentTransformMode == TransformMode.Position)
            {
                HandleGrabPosition();
    ...
    {
        Vector3 currentHandPos = usingLeftHand ? leftPinchPos : rightPinchPos;
    ...
    void HandleRotation()
    {
        Quaternion currentHandRotation = usingLeftHand ? leftHandRotation : rightHandRotation;
        ...
        // Left hand: use rotation as-is (correct)
        // Right hand: invert all axes to mirror the rotation
        if (!usingLeftHand)
        {
    ...
    void UpdateGuideText()
    {
        ...
        {
            InteractionMode.None => $"[{modeName} MODE] Pinch to grab | Both: Stretch",
            InteractionMode.SingleHand => currentTransformMode == TransformMode.Position ? "Moving" : "Rotating",
            InteractionMode.BothHands => "Stretching",
            _ => ""
        };
        
        guideText.text = $"{interactionText}\nDouble Pinch: Toggle Mode | Clap: Rotate Space";
    }
